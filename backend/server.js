...
apiRouter.post('/login', async (req, res) => {
    ...
    const user = users.find(u => u.username === username && u.password === password && u.class_code === class_code);
    ...
    if (user) {
        ...
        res.json({
            ...
            user: { // Send relevant user data, exclude password
                id: user.id,
        ...
        res.status(401).json({ detail: 'Invalid credentials or class code' });
    }
});
